	...
	<div id="container" class="_product_index">
		<div id="content">
			<div >
				...
				<div class="hd_wrap">
					<h4 class="hd3 fl">수다톡 검색</h4>
					<span class="dsc_txt"><span class="f12">※</span> 공지사항을
						검색하세요.</span>
				
				<form action="TB_noticeManage" method="post" class="test">
					<div class="srch_tmp">
						<div>
							<div>
				...
				<div class="uio_grid" style="width:100%;height:440px">
					...
					<table width = "100%" class="nb" >
			         ...
			         <c:forEach items="${ list }" var="dto">
			            <tr class="thead-inverse">                  
			               <td>${dto.t_no }</td>
			               <td><a href="TB_content_view?n_no=${dto.t_no }">${dto.t_title }</a></td>
			               <td>${dto.t_writer }</td>
			               <td>${fname }</td>
			               <%-- <td>${dto.n_regdate }</td> --%>
			               <td>
			               		<input type="button" value="삭제" onclick="location.href='TB_delete?t_no=${dto.t_no}'">
                  				<input type="button" value="수정" onclick="location.href='TB_modify_view?t_no=${dto.t_no}'">
                  		   </td>
			            </tr>
			         </c:forEach>   
			      </table>
		         ...
		         <table width="100%">
		            <tr>
		               <td align="center">
		                  <!-- 처음 이전 링크 --> 
		                  <c:if test="${pg>block}">
		                     <a href="TB_noticeManage?pg=${fromPage-1}&dataFrom=${dataFrom}&dataTo=${dataTo}&category_selector=${category_selector}&category_option=${category_option}&dataForSearch=${dataForSearch}"><img
		                        src="<c:url value='resources/img/arrowsleft.gif'/>"></a>
		                     <a href="TB_noticeManage?pg=${pg-1}&dataFrom=${dataFrom}&dataTo=${dataTo}&category_selector=${category_selector}&category_option=${category_option}&dataForSearch=${dataForSearch}"><img
		                        src="<c:url value='resources/img/arrowleft.gif'/>"></a>
		                  </c:if> 
		                  <c:if test="${pg<=block}">
		                      <c:if test="${pg>1}">
		                           <img src="<c:url value='resources/img/arrowsleft.gif'/>">
		                           <a href="TB_noticeManage?pg=${pg-1}&dataFrom=${dataFrom}&dataTo=${dataTo}&category_selector=${category_selector}&category_option=${category_option}&dataForSearch=${dataForSearch}">
		                           <img
		                          src="<c:url value='resources/img/arrowleft.gif'/>"></a>
		                      </c:if>   
		                  ...
		                  <c:forEach begin="${fromPage}" end="${toPage}"
		                     ...
		                     <c:if test="${i!=pg}">
		                        <a href="TB_noticeManage?pg=${i}&dataFrom=${dataFrom}&dataTo=${dataTo}&category_selector=${category_selector}&category_option=${category_option}&dataForSearch=${dataForSearch}">${i}</a>
		                     </c:if>
		                  </c:forEach>  
		                  <!-- 다음, 이후 --> 
		                  <c:if test="${toPage<allPage}">
		                     ...
		                     <a href="TB_noticeManage?pg=${toPage+1}&dataFrom=${dataFrom}&dataTo=${dataTo}&category_selector=${category_selector}&category_option=${category_option}&dataForSearch=${dataForSearch}"><img
